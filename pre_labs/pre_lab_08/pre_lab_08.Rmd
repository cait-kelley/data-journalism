...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. It gives us month names isntead of month numbers. **Answer** Lines of code explanations:

making a new dataframe
that new dataframe shows month as written names not numbers
it also has two columns, month
and the second column is total_calls
the dataframe is organized most calls to least
...
# saving this out to another dataframe is very important because we are putting this new dataframe in ggplot
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We see a blank gray canvas.
...
Using geom_bar() -- as opposed to geom_line() -- says we're making a bar chart. (Don't make pie charts, unless maybe if there are only two things.)
...
After we've invoked the ggplot() function, you'll notice we're using a + symbol. It means the same thing as \|\> -- "and then do this". It's just a quirk of ggplot() that after you invoke the ggplot() function, you use + instead of \|\>. It makes no sense to me either, just something to live with. Weight is the size of the bars in the bar chart.

### Task 5: Run the following code

**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We see a simple bar chart in chronological order by the x-axis, left to right by month starting with January. It looks like the bars are shorter in winter months but it's a bit unclear.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now the x-axis is ordered by size, not chronologically. It's clear that there are more calls March-Aug than Sept-Feb. Though the most calls are in March, May and August and I'm not exactly sure why that is.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Coord_flip just switches the axes. Now the months are on the y-axis and total calls are on the x-axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The color behind the bars is gone, otherwise it's the same.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** Blue background!
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Nice blue background. Much better labels. Data source.
...
So, let's use the same Baltimore County data and generate a count of calls for each date in our data.

### Task 12: Create a dataframe of 911 calls by date

**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We are making a new dataframe that we can plug into ggplot. I just see a summary of the new dataframe with columns called data and total_calls and 366 rows.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We see a very spiky line chart that shows calls per day. But there's not a lot of detail on the labels of the x-axis which makes it hard to read. I think an average line would be easier to read.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We broke it. The labels on the x-axis all overlap. If we could read the labels and tick marks it could be helpful though.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The last line of code angles the x-axis labels so they are almost readable. Making the text rotate more makes it more readable. You can still kind of see the calls drop off a bit in winter.
