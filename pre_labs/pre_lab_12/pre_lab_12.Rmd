...
#save it as a variable so if the url changes or you change the url you want to use then you only have to change it in the code once
```
...
# html_table extracts any tables on a page
# show the dataframe
...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The column name's aren't clean and the date column is a character column.
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** It could be helpful to save the urls somehow?
...
tags there are in the HTML source? **Answer** ul is unordered list. There are multiple lists.

```{r}
osp_results |> html_elements('li')
#elements plural, without s only granbs the first element you specify. But here we want all the li tags.
```
...
tags. Describe the output: **Answer** It shows a list of just the text in the li tags.

```{r}
osp_results |> html_elements('li') |> html_text()
#we just want the text in the li tags
```
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** This made a dataframe called releases! We will still need to isolate the list items that have the names of the press releases.
...
#this isolates the listing items
```
...
#repeating some things. now it also uses the function called separate. it takes the value column and separates it into two columns called data and title. The separator is a colon.

#the error message actually shows us the rows with meeting minutes because those didn't have colons.
```
...
#getting rid of na rows entirely. and getting rid of trailing spaces with str_squish
```
