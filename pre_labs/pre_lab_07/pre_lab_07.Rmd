...
```{r echo=FALSE}
census_api_key("133531fbde4de86ffa7dd0c9242b66b3ebd7c5ab", install=TRUE)
```
...
geography means the level, state, city, etc.
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada is the state with the largest percentage growth between 2010 and 2000.
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** MD county with highest median household income: Howard County

```{r}

#pick your geography, choose a state and a year (essentially the end year of the ACS, 2021 is most recent)

#moe = margin of error. in reporting say "about," "roughly"
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Estimated median household income by county in Maryland, ranked.
...
Is the difference between Calvert and Montgomery significant? **Answer** Either could be the second most median household incomes.

Is the difference between Howard and everyone else significant? **Answer** Yes because the MOE doesn't overlap with any other MOEs.
...
What's one question we could ask of this data now that we have enrollment percentages and household income for the high school's zip code? What might make the answer harder to interpret? **Answer**

(Worked with April, the woman, the legend.)

Question: At what approximate median income do more than 50% of highschool graduates enroll in college after graduating? The margin of error and outliers could make this hard to interpret. Bard High School Early College in Baltimore City is an outlier. The median income is roughly 40,000 but more than 50% of highschool graduates enroll in college. This makes sense because it allows highschool students to take college courses for free.
...
**Answer** There are 50 high schools that have 60% or more graduates immediately enrolling in college and the average median income of the zip codes where those high schools are located is 111,448.3. There are 76 high schools with less than 40% of graduates enrolling in college. The average median income of the zip codes of those high schools is 69,252.2.
...
**Task** What is another question you could ask of this data? **Answer** Do high schools that focus on technical skills have higher rates of college enrollment for their graduates than non-technical focused high schools? I could check the schools with "technical" or "tech" or other similar terms in their names.
