...
There's zip code information and mapping data. One column shows the full 9-digit zip code that tells us what state we are talking about and one just has the 5-digit zip codes. There are columns for shape area and length and a column called geography that has coordinates. R considers the data in that column multipolygon data, not character, number, or time data. The existence of that column lets us create a map.
```{r}
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** This takes me to the dataframe md_zips. There are 526 unique 9-digit zip codes (not all the 5-digit ones are unique) in Maryland. The dataframe has 8 columns and 526 rows.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows a map of Maryland! The latitude is the x-axis and the longitude is the y-axis. The map is divided into zip codes.
...
**Task** Run the following code. Describe the output in the space below. **Answer** I think this is a repeated question, I'll answer below
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's information about all US counties. County name, county name with words like "county" or "municipio" or "parish" included, state FIPS code, GEOID, other columns I don't understand and the geometry column that allows us to make a map of all counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created a new dataframe with only the rows that pertain to Maryland. There are 24 rows because there are 24 counties in Maryland (including Baltimore City).
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now there is a map of Maryland divided by county with the x-axis still showing latitude and the y-axis still showing longitude. The counties are easier to see and more recognizable than the zip codes, but you can't see the Chesapeake Bay which is a little confusing.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The zip code map has been layered on top of the county map. It allows us to see how some boundaries extend into water.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It created a dataframe called foreclosure_zip_march_2023 that has two columns: zip and march_2023. It has 547 rows. The zip codes are correctly considered characters and the number of notices in march are correctly considered numbers.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The March foreclosure notice dataframe was added as a column to the maryland zip codes dataframe and made into a new dataframe. They were joined by the 5-digit zip code column (which makes sense because those codes don't change. The additional 4 zip code numbers are regularly adjusted by the postal service.)
...
**Task** Run the following code. Describe the output in the space below. **Answer** There is a very pretty gradient applied to the map. Now the zip codes with the most foreclosure notices in March 2023 have the lightest blue color and the blue gets darker as counties have fewer notices. The most notices were in the Baltimore City area and Prince George's County.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** I don't think I need to put in my API key again and there's nothing really to answer here.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We created two dataframes: md_county_population and md_county_notices. The notices dataframe has one extra row with now county name so I don't know why that's there. If we combine these dataframes by the column "county" we will have to clean the names of the counties and the column names so the columns match.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This added a geometry column to md_county_population.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code just shows us the dataframes and the information in the dataframes. We can see the county information doesn't match.
...
Now md_county_population only has three columns: county, population and geometry. And now the county names are title case and include the word "County" but nothing else.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We joined the two dataframes by the column "county" and made a new dataframe with the combined information.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This added a column called "rate" that shows the number of notices per 1000 people (why do we use 1000 people instead of 100 people? I guess the numbers would get really small with 100, but is there an advantage to be able to talk about percentages?).
...
**Task** Run the following code. Describe the output in the space below. **Answer** I see a beautiful gradient map. Very nice blue colors. It shows the most notices per 1000 people were in Charles County. However, it is hard to tell the difference between the darker shades.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is much easier to read. Although I'd prefer a dark purple, not black. The black stands out as too different.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** It shows the most notices in Charles County with a fair number in the PG area and Baltimore City and a few other counties. MOntgomery and Garrett counties stand out as having the least.
