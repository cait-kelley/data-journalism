...
It's a dataframe with 4 columns and 999 rows. The columns are data, title, url and text. They show press releases of Ben Cardin. 
```{r}
...
#. means any character that comes after the p
```
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It shows how many unique words are in the list, but it is case sensitive so you should standardize the text case first.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** This made a dataframe with hundreds of thousands of rows showing every unique word and character that looks like a word.
...
#tokens means words and any individual characters for linguistic people
#this is saying pull apart all the sentences and make a list of all unique words
```

### Task 7: Make a column chart

**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It's not that interesting because it makes a chart that shows the most basic words in the text, "the" etc.
...
```{r}
data("stop_words")
#stop words are filler words and you can add words to this list that are fillers or not interesting in your case
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes! It shows the percent of the total words that these words represent. That doesn't matter a lot though, what matters is the distance between the percentages and comparing which words are used the most or least.
...
**Task** Run the following code and describe the results. **Answer** We created three dataframes that compare the top ten words in his press releases in 2021, 2022 and 2023.
...
**Task** Run the following code and describe the results. **Answer** Now we see a list of common words that are commonly used next to each other. Some naturally go together and some don't, so be careful. These results get at clearer ideas and concepts.
...
**Task** Run the following code and describe the results. **Answer** The top results were project location and amount included, which aren't that interesting. Health care seems very important to him, it's number three on the list.
...
**Task** Run the following code and describe the results. **Answer** Human rights and equal rights are now the top two things, maybe after Dobbs decision? Health care still number three. Interesting that he mentions oral health a lot.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** This is a challenging tool to use because words change meaning with context or time. This creates dataframes with a column that assigns positive or negative to each word.
