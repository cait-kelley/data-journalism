...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** R assumed the first line were column names. There are also many periods after the county and region names.
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** Now R has made generic column names called x1, x2 etc. And there are still lots of periods in the first column.
...
**Task** Run the codeblock below. What else do we need to do? **Answer** We still have to remove the periods.
...
#jurisdiction column str_replace_all: the backslashes are necessary for a literal period because a period means something else in R. replacing the period with nothing aka ''. str_squish gets rid of leading or trailing spaces.
