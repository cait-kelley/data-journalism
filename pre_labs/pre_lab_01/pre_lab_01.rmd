...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** data type for the id column: character <chr>, data type for seats column: <dbl> double precision, a number without decimals
...
**Answer** The first line of code identified the data set we are working with. The second line showed all the different terms with no repeats. Then the summarize and count classes showed how many rows/classes are in each term.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Now the results are organized by the number of classes, highest to lowest instead of the term, oldest to newest.
...
**Answer** This output included department and not just term and number of classes. arrange (term, department)
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** We calculated total seats and arranged the information by descending total seats instead of term and department.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** This codeblock added the mean and median seats for each term. The information is still arranged by total numbers of seats in descending order. The top terms are all from fall; more classes are offered in fall terms.
...
**Answer** It adds minimum and maximum seats and groups initially by department. That changes the first column to department. The total number of seats column has much smaller numbers now, understandably.
...
1.  What department has the smallest average class sizes?

2.  What departments cut the most classes during the pandemic?

3.  Did all departments get back to pre-pandemic numbers of classes offered?
