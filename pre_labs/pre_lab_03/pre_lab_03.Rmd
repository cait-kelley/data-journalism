...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No it doesn't show any parsing errors now. 
...
**Task** Run the following codeblock to glimpse the data. It lets you check quickly if your columns are being categorized properly. The backwards single quotation marks are what R does when there are spaces in column names. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer**
"Vendor Name" is a character field. "Amount" is a number field. "Date" is a character field.
...
**Answer** 549 zip codes do not have five characters. We learned that some of those are missing zeros in front of four characters. And some are international postal codes.
...
**Answer** 
Read md_grants_loans
Organize by the two columns Fiscal Year and Category
Show how many rows match each fiscal year and category combo
list fiscal year as the first column and in ascending order

It's problematic that some categories say NA or something other than a loan or grant. It's also problematic that 2009 lists no loans.
...
**Answer** The smallest amount is $60 given to Land Preservation Trust, Inc by the Dept of Natural Resources/Rural Legacy Program. There are only two amounts less than $100 and one between $100 and $300. Then the amounts increase relatively steadily.
...
**Answer** It removes spaces in the column names and replaces them with underscores. It also made the names lowercase.
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. You can standardize however you want, but you want consistency.
**Answer** 
First line updates the new dataset we just made called cleaned_md_grants_loans from the md_grants_loans dataset
then it takes the spaces out of the column names and makes them lowercase
then it changes the column name grantor to source
then it changes all the column text of the columns "source," "grantee," and "description," to all uppercase
Then we see the changes
...
**Answer** 50 are possible, but these don't seem to be true duplicates. These are examples of organizations getting multiple grants, or two payments from a grant, per year.
...
We want a new column called zip5. We want to pull just the first five numbers of each zip code and put them in the new column. String sub means give me a portion of this thing.
**Answer**
We got a new column called zip5 that pulled just the first five numbers of each zip code.
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. Use filter, group by and summarize.

which zip code, use column zip5
...
(this line tells the code what dataset to pull from) 
cleaned_md_grants_loans |>
 
(this pulls only rows that say MARYLAND TOURISM BOARD in the source column)
 filter(str_detect(source, "MARYLAND TOURISM BOARD"))|>
 
(this makes the first column zip5) 
  group_by(zip5)|>

(this makes a new column called total that shows the total amounts for each distinct zip code)
  summarize (total = sum(amount)) |>
  
(this shows the data with the largest amount of money first)
  arrange(desc(total))
...
**Answer** The zip code that has gotten by far the most amount of money from the Maryland Tourism Board is 21202. The zip code received $846,000 and is part of downtown Baltimore (and also a small area in the Bayview neighborhood?). But this does make sense because downtown Baltimore is a tourist destination. There are museums and a waterfront there for example. And Baltimore is the biggest city in Maryland by far so they will spend more money and probably have more tourists or visitors.
