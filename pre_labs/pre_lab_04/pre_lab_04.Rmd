...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are lots of similar names like, "WASHINGTON CO COMMISSIONE" and "WASHINGTON CO COMMISSIONERS."
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. (When you mutate and create a new column the new column ends up at the end of the data frame. But the select line of code says to put Grantee, then grantee_clean, then the rest of the columns in that order.) **Answer**

1. "Melwood Horticultural Training Center, Inc" to "Melwood Horticultural Training Center"
2. "Montgomery County Chamber Comm Foundation Inc" to "MONTGOMERY COUNTY CHAMBER COMM FOUNDATION INC"
...
1."WASHINGTON ADVENTIST UNIVERSITY" to "Washington Adventist University"
2. "WASHINGTON CO COMM. ACTION COUNCIL INC" to "WASHINGTON COMM ACTION COUNCIL"
3. "WASHINGTON COLLEGE" to "Washington College"
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No it doesn't make substantially different changes that I can see in the first few pages.
