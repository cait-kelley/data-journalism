...
And just like that, we have just Journalism results, which we can verify looking at the head, the first six rows. We also could filter by a numeric column like `term`, so we could see just courses in 2023: (remember a numeric field doesn't need quotation marks, only character fields need quotation marks.)
...
We also have more columns than we might want. For example, we may only want to work with the course id and title. Filter limits the rows you see, select filters what columns you see.
...
**Answer** Making a new variable is important to save the new dataset we made when we filtered out information. We want to store these specfic results in an accessible place so we don't have to retype the code and we don't want to overwrite the original dataset.
...
**Answer:** We changed the date format so that R knows the information is a date. The column expenditure_date now says <date> underneath it and the dates are formatted year-month-day instead of month/day/year.
...
**Answer** Because we asked R to organize the information by month, it needed a way to combine all the data from each day inside each month. Instead of doing what I might do and naming each month (May, June, etc.) R assigned all the data from all days in each month to the first day of each month to keep the data in number format. That way we can see the total expenditures for each month.
...
**Task** Let's add a new column based on an existing column. Run the following code to create a new column called `total_votes` based on the votes cast for each candidate, and then to create a new column called pct_moore.
...
**Answer** first we are saving these new calculations and these new columns to the dataset itself so our original dataset will have these new columns from now on. Then we are telling the dataset to make a new column of total votes and explaining this column is the sum of the cox, moore, lashar, wallace, and write in columns. Then we make a new column for the percent of the vote moore won and explain that is the moore column divided by the total votes column.
...
**Answer** When comparing Task 10 and Task 12, the percentage of votes Moore got are shown as a percentage, not a decimal because I put "*100" after the division.
...
**Answer** Both are equally intersting. In news we like to look at extremes, so I'd like to know in what counties Moore did worst and best in.
...
str_to_title() converts to title case, so only the first letter of each word is capitalized.

str_to_sentence() convert to sentence case, so only the first letter of sentence is capitalized.
...
**Answer** The mutate statement is looking in the rows in the address column for full state names and then making a new column called "state" that identifies each row as MD, CA, WA, LA, FL, NC, MA, WV, VA, or NA. We put West Virginia in the code first so that all instances of West Virginian addresses would be accounted for and organized BEFORE we counted addresses that just say Virginia. I don't know if that would result in addresses being double counted, or if all the WV and VA addresses would go in the WV column and zero would go in the VA column. But either way it would be bad.
...
**Answer** Yes it makes sense that most purchases would be in-state in Maryland. However, the WA label is problematic because it includes Washington, DC addresses and Washington state addresses. It also makes sense that California would be high on the list because a lot of tech companies are based there, Paypal, Zoom etc.
...
1. I saw some expenses were "consulting fees." I'd be interested to know more about what that consulting entails and how much committees spent. Which committees spent the most on consulting fees?

2.  In what cities did the committees spend most of the money?

3.  What professors taught the most cross-listed courses?
