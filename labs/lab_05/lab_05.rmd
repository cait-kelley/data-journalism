...
```{r}
baltimore_county_911<-read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> 
  clean_names()  |> 
  rename(date = x1, time = x2, case_number = x3, event_type = x4, location = x5)

baltimore_county_911 <- baltimore_county_911 |> mutate(date=mdy(date))

baltimore_county_911
```
...
A1. July 14, 2022 and October 4, 2022 had the most overdose calls: 23 each day. I checked the beginning date and end date of the data, the data goes from Feb 6, 2022-Feb 6, 2023 which totals 366 days. There are 366 rows of data which means at least one overdoes call occured on each day of the year in the dataset.
...
A2. I asked Chat GPT how to make a column of weekdays: "How do I add a column to a dataframe in R Studio that displays what day of the week each date represents using the library lubridate and tidyverse?"

Saturday has the most calls: 15.5% of all calls, followed closely by Sunday: 15.1%. Thursday has the least amount of calls: 12.8%. There are more than 100 more calls on Saturdays (638) than there are on Thursdays (526). It's interesting that Tuesday has the most calls outside the weekend, not Monday or Thursday, the days that border the weekend.

```{r}
baltimore_county_911<-baltimore_county_911|>
  mutate(day_of_week = wday(date, label = TRUE, abbr = TRUE))

total_calls <-baltimore_county_911 |>
  summarize(total = n()
)

baltimore_county_911_by_day<-baltimore_county_911|>
  group_by(day_of_week)|>
  summarise(
    count_calls = n(),
    percent_calls = (count_calls/total_calls$total)*100
  )|>
...
A3.4540 SILVER SPRING RD has the most calls: 36

Also the next two highest at 13 calls: PC 02; 6424 WINDSOR MILL RD and PC 06; 115 SUSQUEHANNA AV W

The addresses are reported in different formats, maybe different precincts enter the data differently.
...
A4.I want to investigate that house! It's now listed on Zillow for $400,000. Unclear if it's the same house, or the old one was torn down.
